$(function(){
	...
	function displayResult(item) {
        $("#txt_id_").val(item.value);
    }
	
	function displayResultCodigo(item) {
        $("#txt_id_").val(item.value);
    }
	
	function displayResultReferencia(item) {
        $("#txt_id_").val(item.value);
    }
	...
	$(".add-carrito-venta").on("click", function(e) {
		...
		$.ajax({
			...
			if(data.success==true){
				...
				$(".detalle--venta").load(ENV_WEBROOT+'view/Operacion/carrito-venta.php');
				setTimeout(function() {
					if(tipo_doc!=1){
						$(".hide-tipo-doc").hide();
	...
	$(".txt-precio-").off("focusout");
	$(".txt-precio-").on("focusout", function(e) {
		var precio = $(this).val();
		var id = $(this).attr('_id');
		var tipo_doc = $("#cbo-tipo-documento").val();
		$.ajax({
	...
	$(".cbo-tipo-precio").on("change", function(e) {
		...
		$.ajax({
			...
			type: 'post',
			data: {'id':id,'tipo_precio_id':tipo_precio_id},
			dataType: 'json'
		}).done(function(data){
			if(data.success==true){
				$(".detalle--venta").load(ENV_WEBROOT+'view/Operacion/carrito-venta.php');
